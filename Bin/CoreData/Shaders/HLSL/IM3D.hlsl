...
cbuffer cbContextData : register(b5)
{
    float2   cViewport;
};
#endif
...
VS_OUTPUT VS(VS_INPUT _in) 
{
    ...
    ret.m_position = GetClipPos(_in.m_positionSize.xyz);
    return ret;
}
#endif
        ...
        [maxvertexcount(4)]
		void GS(point VS_OUTPUT _in[1], inout TriangleStream<VS_OUTPUT> out_)
		{
			VS_OUTPUT ret;
			
			float2 scale = 1.0 / cViewport * _in[0].m_size;
	 ...
	 // expand line -> triangle strip
		[maxvertexcount(4)]
		void GS(line VS_OUTPUT _in[2], inout TriangleStream<VS_OUTPUT> out_)
		{
			float2 pos0 = _in[0].m_position.xy / _in[0].m_position.w;
			float2 pos1 = _in[1].m_position.xy / _in[1].m_position.w;
		
			float2 dir = pos0 - pos1;
			dir = normalize(float2(dir.x, dir.y * cViewport.y / cViewport.x)); // correct for aspect ratio
			float2 tng0 = float2(-dir.y, dir.x);
			float2 tng1 = tng0 * _in[1].m_size / cViewport;
			tng0 = tng0 * _in[0].m_size / cViewport;
