#include "BRDF.hlsl"
	...
	float3 GetBRDF(float3 worldPos, float3 lightDir, float3 lightVec, float3 toCamera, float3 normal, float roughness, float3 diffColor, float3 specColor)
	{
        ...
        float3 specularFactor = 0;
            ...
            //else
            //{
                //half invRough = 1 - roughness;
                //half specular_exponent = exp2(1 + invRough * 9);
                //half D = .5h * ( specular_exponent + 1 ) * pow( ndh, specular_exponent );
                //half FG = .25h / ( pow(ldh, 3) + (0.03));
                //specularFactor = specColor * D * FG * ndl/M_PI;
